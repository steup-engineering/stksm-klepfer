...
cp target/stksm-gui-*.jar "$LINUXCNC_HOME/java/stksm-klepfer-gui.jar"
cp -r target/lib "$LINUXCNC_HOME/java"

mkdir -p "$LINUXCNC_HOME/bin"
cp script/launch.sh "$LINUXCNC_HOME/bin/stksm-klepfer-gui"
chmod ugo+x "$LINUXCNC_HOME/bin/stksm-klepfer-gui"
